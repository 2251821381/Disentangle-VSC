...
from evalution.utils import batch_mel_spectral_distortion
from evalution.text_to_speech import batch_mel_cepstral_distortion
...
def print_results(results, show_bin):
    ...
    def _print_result(results):
        ...
        res = {
            "nutt": len(results),   # 样本数量
            "dist": dist,           # 总的距离
            "dur_ref": int(dur_ref),    # 参考语音的总时长
            "dur_syn": int(dur_syn),    # 合成语音的总时长
            "dur_ali": int(dur_ali),    # 对齐后的语音的总时长
            "dist_per_ref_frm": dist / dur_ref, # 每个参考语音帧的平均距离
            "dist_per_syn_frm": dist / dur_syn, # 每个合成语音帧的平均距离
            "dist_per_ali_frm": dist / dur_ali, # 每个对齐后语音帧的平均距离
            "ins": nins / dur_ref,  # 插入错误数量
            "del": ndel / dur_ref,  # 删除错误数量
        }
        print(tabulate(
            [res.values()],
            res.keys(),
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--path", type=str, default="eval_data_pairs_p225_p226.csv")
    parser.add_argument("--path", type=str, default="eval_data_pairs.csv")
    parser.add_argument("--mcd", type=bool, default=True)
    parser.add_argument("--msd", type=bool, default=True)
    parser.add_argument("--show-bin", type=bool, default=False)
    args = parser.parse_args()

    main(args.path, args.mcd, args.msd, args.show_bin)
