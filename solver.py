...
class Solver(object):
    ...
    def loss_fn(self, original_seq, recon_seq, recon_seq_psnt, c_mean, c_logvar):
        ...
        kld_c = -0.5 * torch.sum(1 + c_logvar - torch.pow(c_mean, 2) - torch.exp(c_logvar))
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            c_mean, c_logvar, content, recon_mel, recon_mel_psnt = \
                self.G(x_real_org, x_real_org, f0_org_intrp, emb_org, emb_org, x_real_org)
            g_loss_id, g_loss_id_psnt, kld_c = self.loss_fn(x_real_org, recon_mel, recon_mel_psnt, c_mean, c_logvar)

            g_loss = g_loss_id + g_loss_id_psnt + kld_c
                ...
                with torch.no_grad():
                    ...
                    for val_sub in validation_pt:
                        ...
                        for k in range(4, 5):
                            ...
                            _, _, _, _, x_identic_psnt = \
                                self.G(x_real_pad, x_real_pad, f0_org_val, emb_org_val, emb_org_val, x_real_pad)
                    ...
                    for val_sub in validation_pt:
                        ...
                        for k in range(4, 5):
                            ...
                            _, _, _, _, x_identic_woF = \
                                self.G(x_real_pad, x_real_pad, torch.zeros_like(f0_org_val), emb_org_val, emb_org_val, x_real_pad)
                            # 合成的mel且Rhythm为空
                            _, _, _, _, x_identic_woR = \
                                self.G(x_real_pad, torch.zeros_like(x_real_pad), f0_org_val, emb_org_val, emb_org_val, x_real_pad)
                            # 合成的mel且Content为空
                            _, _, _, _, x_identic_woC = \
                                self.G(torch.zeros_like(x_real_pad), x_real_pad, f0_org_val, emb_org_val, emb_org_val, x_real_pad)
                            # Remove Timbre
                            _, _, _, _, x_identic_woU = \
                                self.G(x_real_pad, x_real_pad, f0_org_val, emb_org_val, torch.zeros_like(emb_org_val), x_real_pad)
                            # Remove Accent
                            _, _, _, _, x_identic_woA = \
                                self.G(x_real_pad, x_real_pad, f0_org_val, emb_org_val, emb_org_val, torch.zeros_like(x_real_pad))
                            ...
                            fig, ((ax1, ax3), (ax4, ax5), (ax6, ax7)) = plt.subplots(3, 2, sharex=True, sharey=True)
                            ...
                            plt.savefig(f'{self.sample_dir}/{i + 1}_{val_sub[0]}_{k}.png', dpi=600)
                            plt.close(fig)
